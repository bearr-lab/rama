...
export function TrustBadge({ status, variant = "solid", className }: TrustBadgeProps) {
  const config = {
    verified: {
      icon: ShieldCheck,
      text: "DLD Verified",
      ...
      tooltip: "Price and details matched with official Dubai Land Department records.",
    },
    review: {
      icon: Clock,
      text: "Under Review",
  ...
  const { icon: Icon, text, color, bgSolid, bgOutline, tooltip } = config[status]
  ...
  return (
    <Tooltip>
      <TooltipTrigger>
        <Badge 
          className={cn(
            "flex items-center gap-1.5 font-medium border cursor-help transition-colors",
