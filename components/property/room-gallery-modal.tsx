          ...
          <div className="flex size-full items-center justify-center bg-zinc-900">
             ...
             <div className="relative flex aspect-video w-full max-w-3xl items-center justify-center overflow-hidden rounded-xl border-2 border-fjord/40 bg-zinc-950">
                ...
                <div className="absolute top-1/4 left-10 h-2/3 w-1/2 rounded-sm border border-fjord/50">
                   <span className="absolute top-2 left-2 font-mono text-xs text-fjord">LIVING</span>
                </div>
                <div className="absolute top-1/4 right-20 h-1/2 w-1/3 rounded-sm border border-fjord/50">
                   <span className="absolute top-2 left-2 font-mono text-xs text-fjord">BEDROOM</span>
                </div>
             </div>
          </div>
        ) : (
      ...
      <div className={cn(
        ...
        <div className="pointer-events-auto absolute inset-x-0 top-0 flex h-32 items-start justify-between bg-linear-to-b from-black/80 to-transparent p-6 pr-24">
          ...
          <div className="mr-16 flex items-center gap-3">
             ...
             <div className="flex rounded-md border border-white/10 bg-black/40 p-1 backdrop-blur-md">
                <button className="rounded p-2 text-white/70 transition-colors hover:bg-white/10 hover:text-white" title="Save">
                  <Heart className="size-4" />
                </button>
                <button className="rounded p-2 text-white/70 transition-colors hover:bg-white/10 hover:text-white" title="Share">
             ...
             {[
               ...
               return (
                 <button
                   key={tab.id}
                   onClick={() => {
                     setSelectedCategory(tab.id as 'all' | 'kitchen' | 'bedroom' | 'living' | 'balcony');
                     setCurrentIndex(0);
                     setActiveHotspot(null);
                   }}
                   className={cn(
                   ...
                   {!isActive && (
                     <div className="pointer-events-none absolute bottom-full left-1/2 mb-4 flex -translate-x-1/2 gap-2 opacity-0 transition-opacity group-hover:opacity-100">
                       {images.filter(img => tab.id === 'all' || img.category === tab.id).slice(0,3).map(img => (
                         <div key={img.id} className="relative h-16 w-24 overflow-hidden rounded-md border border-white/20 shadow-xl">
        ...
        <div className="pointer-events-auto absolute bottom-8 left-8 flex flex-col items-start gap-4">
           ...
           <div className={cn(
             ...
             isMetricsExpanded ? "max-h-96" : "max-h-24 cursor-pointer hover:bg-[#161616]/90"
           )} onClick={() => !isMetricsExpanded && setIsMetricsExpanded(true)}>
              <div className="flex items-center justify-between border-b border-white/5 p-4">
                 <div>
                   <p className="mb-1 text-[10px] font-bold tracking-wider text-white/60 uppercase">Asking Price</p>
                   <div className="flex items-baseline gap-1">
                     <span className="font-mono text-xl font-bold">AED 4.5M</span>
                   </div>
                 </div>
                 {isMetricsExpanded ? (
                   <button onClick={(e) => { e.stopPropagation(); setIsMetricsExpanded(false); }} className="z-10 p-2 text-white/60 hover:text-white">
                     <ChevronDown className="size-4" />
                   </button>
                 ) : (
                   <div className="text-right">
                     <p className="text-xs font-bold text-emerald-400">+7.2% ROI</p>
                   </div>
                 )}
              </div>
              
              <div className={cn("space-y-4 p-4", isMetricsExpanded ? "opacity-100" : "pointer-events-none h-0 p-0 opacity-0")}>
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-[10px] tracking-wider text-white/50 uppercase">Price / Sqft</p>
                      <p className="font-mono text-sm font-medium">AED 2,150</p>
                    </div>
                    <div>
                      <p className="text-[10px] tracking-wider text-white/50 uppercase">Cap Rate</p>
                      <p className="font-mono text-sm font-medium">6.8%</p>
                    </div>
                    <div>
                      <p className="text-[10px] tracking-wider text-white/50 uppercase">Est. Rental</p>
                      <p className="font-mono text-sm font-medium">324K/yr</p>
                    </div>
                    <div>
                      <p className="text-[10px] tracking-wider text-white/50 uppercase">Service Chg</p>
                ...
                >
                  <span className={cn(
                    ...
                    <span className={cn(
                      ...
                      isActive ? "scale-150 border-emerald-300 bg-emerald-500" : "bg-white group-hover:bg-emerald-400"
                    )} />
                  </span>
                ...
                {isActive && (
                  <div className="animate-in fade-in zoom-in-95 absolute top-6 left-1/2 z-30 w-64 -translate-x-1/2 rounded-xl border border-white/20 bg-black/70 p-4 shadow-2xl backdrop-blur-xl duration-200">
                    <div className="mb-2 flex items-start justify-between">
                       ...
                       <button onClick={() => setActiveHotspot(null)} className="-mt-2 -mr-2 p-2 text-white/50 hover:text-white">
                         <X className="size-3" />
                       </button>
                    </div>
                    {hs.brand && (
      ...
      {showHint && (
        <div className="animate-in fade-in pointer-events-none absolute inset-0 z-50 flex items-center justify-center duration-500">
           <div className="flex items-center gap-3 rounded-full border border-white/10 bg-black/60 px-6 py-3 text-sm font-medium text-white/90 shadow-2xl backdrop-blur-md">
             ...
             Use <kbd className="mx-1 rounded bg-white/20 px-1.5 font-mono text-xs">←</kbd> <kbd className="mr-1 rounded bg-white/20 px-1.5 font-mono text-xs">→</kbd> keys to navigate · Click pulsing rings for material specs
           </div>
        </div>
      )}
