...
export function DiscoverClient({ locale = 'en' }: { locale?: string }) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-8 p-6 lg:p-10">
      {/* Header & Stats Banner */}
      <header className="flex flex-col justify-between gap-4 border-b border-border/60 pb-6 sm:flex-row sm:items-end dark:border-border/60">
        <div>
          <p className="text-xs font-bold tracking-widest text-fjord uppercase dark:text-muted">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-relaxed font-light text-muted dark:text-muted">
        ...
        <div className="flex flex-row flex-wrap items-center gap-3">
          <div className="flex flex-row items-center gap-3.5 rounded-none border border-border/80 bg-surface px-4 py-2 text-xs shadow-2xs sm:gap-4 dark:border-border/80 dark:bg-fjord-hover">
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="font-display text-base font-bold text-fjord dark:text-muted">
                <NumberTicker
            ...
            <div className="flex items-center gap-1.5 font-semibold text-fjord dark:text-white">
              <ShieldCheck className="size-4 shrink-0 text-fjord dark:text-muted" />
          ...
          <Button
            ...
            className={cn(
              ...
              showMapDrawer
                ? 'border-transparent bg-fjord-hover text-white hover:bg-surface-subtle dark:bg-surface-subtle dark:bg-surface-subtle'
                : 'dark:border-border-strong border-border/80 bg-surface text-fjord hover:border-border hover:bg-surface-subtle dark:border-border/80 dark:bg-fjord-hover dark:bg-fjord-hover dark:text-white',
            )}
          >
      ...
      {showMapDrawer && (
        <div className="w-full overflow-hidden rounded-none border border-border bg-card shadow-md transition-all duration-300 dark:border-border">
          <div className="flex items-center justify-between border-b border-border bg-surface-subtle/40 px-4 py-2.5 dark:border-border dark:bg-fjord-hover/40">
            <div className="flex items-center gap-2 text-xs font-semibold text-fjord dark:text-white">
              <MapIcon className="size-4 text-fjord dark:text-muted" />
            ...
            <Button
              ...
              className="size-7 text-muted hover:text-fjord dark:text-muted"
            >
              <X className="size-4" />
            </Button>
          </div>
      ...
      <div className="shadow-subtle hover:shadow-floating space-y-5 rounded-none border border-border/40 bg-surface/70 p-6 backdrop-blur-md transition-all duration-300 dark:border-border/40 dark:bg-fjord-hover/70">
        {/* Main AI Semantic Search Bar */}
        <div className="relative flex w-full items-center">
          <div className="pointer-events-none absolute left-5 flex items-center gap-2 font-semibold text-fjord dark:text-muted">
            <Sparkles className="size-4.5 animate-pulse text-fjord dark:text-muted" />
          ...
          <input
            ...
            className="w-full rounded-none border border-border/60 bg-surface/90 px-12 py-4 text-sm font-medium text-fjord shadow-2xs transition-all placeholder:text-muted hover:border-border/40 focus:border-border focus:bg-surface focus:ring-2 focus:ring-stone-900/10 focus:outline-none sm:pl-36 dark:border-border/60 dark:bg-fjord-hover/90 dark:text-muted/70"
          />
          {rawQuery && (
            <button
              onClick={handleClearSearch}
              className="absolute right-4 rounded-none p-1.5 text-muted transition-colors hover:bg-surface-subtle hover:text-fjord dark:bg-fjord-hover dark:text-muted"
              title="Clear search"
            >
              <X className="size-4" />
            </button>
        ...
        <div className="flex flex-col gap-4 pt-1 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <span className="mr-1 flex items-center gap-1.5 font-semibold text-muted dark:text-muted">
              <Sparkles className="size-3.5 animate-pulse text-fjord dark:text-muted" />{' '}
              Suggestions:
            </span>
            <Button
              variant="outline"
              ...
              className="h-7 rounded-none border-border/20 bg-surface-subtle/5 px-3.5 text-xs font-semibold text-fjord shadow-2xs transition-all hover:bg-surface-subtle/15 dark:border-stone-700/20 dark:bg-surface-subtle/5 dark:bg-surface-subtle/15 dark:text-muted"
            >
              High Yield (&gt;7%)
            </Button>
            <Button
              ...
              className="h-7 rounded-none border-border/60 bg-surface px-3.5 text-xs font-medium text-fjord shadow-2xs transition-all hover:border-border/40 hover:bg-surface-subtle hover:text-fjord dark:border-border/60 dark:bg-fjord-hover dark:text-muted"
            >
              Downtown Luxury
            </Button>
            <Button
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border/40 pt-3 lg:border-t-0 lg:pt-0 dark:border-border/40">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {filters.beds !== undefined && (
                <Badge
                  variant="secondary"
                  className="flex items-center gap-1.5 rounded-none border border-border/30 bg-surface-subtle px-3.5 py-1 text-xs font-semibold text-fjord shadow-2xs dark:border-border/30 dark:bg-surface-subtle dark:text-muted"
                >
                  <span>
              ...
              {(filters.community !== 'All Communities' ||
                ...
                <Button
                  ...
                  size="sm"
                  onClick={handleClearSearch}
                  className="h-7 gap-1 text-xs font-medium text-muted hover:text-foreground dark:text-muted"
                >
            ...
            <div className="ml-auto flex items-center gap-3">
              <div className="flex items-center gap-2">
                <span className="hidden text-xs font-semibold text-muted sm:inline dark:text-muted">
                  Sort:
                </span>
                <select
              ...
              <div className="flex items-center rounded-none border border-border/60 bg-surface-subtle/50 p-1 shadow-2xs dark:border-border/60 dark:bg-fjord-hover/50">
                <button
                  onClick={() => setViewMode('grid')}
                  className={cn(
                    'flex items-center gap-1 rounded-none p-1.5 text-xs font-semibold transition-all',
                    viewMode === 'grid'
                      ? 'border border-border/40 bg-surface font-bold text-fjord shadow-2xs dark:border-border/40 dark:bg-fjord-hover dark:text-white'
      ...
      <div className="mt-2 min-h-125 w-full">
        {filteredProperties.length === 0 ? (
          <Card className="flex h-80 w-full flex-col items-center justify-center border-dashed border-border/80 bg-surface/50 p-6 text-center dark:border-border/80 dark:bg-fjord-hover/50">
            <Search className="mb-3 size-10 text-muted dark:text-muted" />
            <h3 className="font-display text-base font-bold text-fjord dark:text-white">
              No Verified Properties Found
            </h3>
            <p className="mt-1 mb-4 max-w-md text-xs text-muted dark:text-muted">
            ...
            <Button
              variant="default"
              onClick={handleClearSearch}
              className="bg-fjord-hover font-semibold text-white shadow-2xs transition-colors hover:bg-surface-subtle dark:bg-surface-subtle dark:bg-surface-subtle"
            >
              Reset All Filters
            </Button>
          </Card>
