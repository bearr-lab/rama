...
const BentoCard = ({
  ...
  <div
    ...
    className={cn(
      ...
      "dark:bg-background transform-gpu dark:border-white/10",
      className
    )}
    {...props}
  >
    ...
    <div className="p-6 z-10 flex flex-col justify-end mt-auto h-full">
      <div className="pointer-events-none z-10 flex transform-gpu flex-col gap-2 transition-all duration-300 lg:group-hover:-translate-y-6">
        ...
        <p className="max-w-lg text-white/80 text-sm drop-shadow-sm">{description}</p>
      </div>
      ...
      >
        <Button variant="link" size="sm" className="pointer-events-auto p-0 text-white font-medium hover:text-white/80" render={<a href={href} />} nativeButton={false}>
          {cta}
    ...
    >
      <Button variant="link" size="sm" className="pointer-events-auto p-0 text-white font-bold hover:text-white/80" render={<a href={href} />} nativeButton={false}>
        {cta}
        <ArrowRightIcon className="ms-2 h-4 w-4 rtl:rotate-180" />
      </Button>
    </div>
