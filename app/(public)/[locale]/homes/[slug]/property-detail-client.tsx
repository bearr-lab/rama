...
export function PropertyDetailClient({ property, locale }: PropertyDetailClientProps) {
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {isScrolled && (
          <motion.div
            ...
            className="fixed top-16 left-0 right-0 z-40 border-b border-border/40 bg-background/90 backdrop-blur-md"
          >
            <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
              <button
                onClick={() => router.back()}
                className="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-muted-foreground transition-colors hover:text-ink"
              >
                <ChevronLeft className="h-4 w-4" />
                {isArabic ? 'العقارات' : 'All Homes'}
              </button>
              <span className="font-display text-sm font-semibold text-ink line-clamp-1 max-w-xs">
                {title}
              </span>
      ...
      <div className="relative h-[calc(100vh-64px)] min-h-[500px] w-full overflow-hidden bg-black">
        ...
        <div className="absolute bottom-6 left-6 z-20 w-[340px] space-y-4">
          ...
          {/* Financial Card */}
          <div className="rounded-xl border border-white/10 bg-[#161616]/95 backdrop-blur-xl p-5 shadow-2xl">
            {/* Header / Price */}
            <div className="mb-6 flex items-start justify-between">
              <div>
                <p className="mb-1 text-[10px] font-bold uppercase tracking-[0.15em] text-white/50">
            ...
            <Button
              ...
              className="w-full rounded-md bg-white py-6 text-sm font-bold text-black transition-transform hover:scale-[1.02]"
            >
      ...
      <div className="bg-background">
        <Container size="lg" padding="lg" className="py-16">
          ...
          <nav aria-label="breadcrumb" className="mb-10 flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>/</span>
            <Link href={`/${locale}/homes`} className="hover:text-ink transition-colors">
              {isArabic ? 'العقارات' : 'Homes'}
            </Link>
            <span>/</span>
            <span className="text-ink font-medium line-clamp-1 max-w-[200px]">{title}</span>
          </nav>
          ...
          <div className="grid grid-cols-1 gap-16 lg:grid-cols-3">
            ...
            <div className="space-y-12 lg:col-span-2">
              ...
              <section>
                <h2 className="mb-5 text-[11px] font-bold uppercase tracking-[0.15em] text-muted-foreground">
                ...
                <div className="grid grid-cols-2 gap-px border border-border/40 bg-border/40 md:grid-cols-4">
                  {property.bedrooms != null && (
                    <div className="flex flex-col gap-2 bg-surface p-5">
                      <BedDouble className="h-5 w-5 text-fjord" />
                  ...
                  {property.bathrooms != null && (
                    <div className="flex flex-col gap-2 bg-surface p-5">
                      <Bath className="h-5 w-5 text-fjord" />
                      <span className="text-2xl font-bold text-ink">
                  ...
                  {property.area_sqft != null && (
                    <div className="flex flex-col gap-2 bg-surface p-5">
                      <Maximize2 className="h-5 w-5 text-fjord" />
                      <span className="text-2xl font-bold text-ink">
                  ...
                  {property.developer && (
                    <div className="flex flex-col gap-2 bg-surface p-5">
                      <Building2 className="h-5 w-5 text-fjord" />
                      <span className="text-sm font-bold text-ink truncate">{property.developer}</span>
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
              ...
              <div className="border border-border/40 bg-surface p-6 space-y-4">
                ...
                <div className="space-y-3 text-xs text-muted-foreground font-light">
                  ...
                  <div className="flex justify-between py-1">
                    ...
                    <span className="font-bold text-ink text-emerald-600">{property.cap_rate_percentage || '6.8'}%</span>
                  </div>
                </div>
                ...
                <Button
                  onClick={() => setIsGalleryOpen(true)}
                  variant="outline"
                  className="w-full font-bold text-xs tracking-widest uppercase py-3 border-fjord text-fjord hover:bg-fjord hover:text-white transition-colors"
                >
                ...
                <Link href={`/${locale}/shortlist`} className="block w-full">
                  <Button className="w-full bg-fjord text-white font-bold text-xs tracking-widest uppercase py-3">
