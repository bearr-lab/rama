...
export async function generateMetadata({
  ...
  return {
    ...
    description: isArabic
      ? 'الوساطة العقارية الفاخرة الرائدة واستشارات الاستثمار في دبي.'
      : 'The leading luxury real estate brokerage and investment advisory in Dubai.',
  };
}
...
export default async function AboutPage({
  ...
  return (
    <>
      ...
      <Section
        spacing="lg"
        className="relative min-h-[60vh] overflow-hidden bg-surface"
      >
        <Container size="xl">
          <div className="grid grid-cols-1 items-center gap-16 lg:grid-cols-2">
            <BlurFade delay={0.25} inView>
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-2 gap-8 border-t border-border/50 pt-8">
                  <div>
                    <h4 className="mb-2 font-display text-3xl font-bold text-fjord">
                      150+
                    </h4>
                    <p className="text-sm tracking-widest text-muted-foreground uppercase">
                      {isArabic ? 'دولة ممثلة' : 'Countries Represented'}
                    </p>
                  </div>
                  <div>
                    <h4 className="mb-2 font-display text-3xl font-bold text-fjord">
                    ...
                    <p className="text-sm tracking-widest text-muted-foreground uppercase">
                      {isArabic ? 'معاملات' : 'In Transactions'}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <BlurFade
              delay={0.5}
              inView
              className="relative flex h-150 items-center justify-center"
            >
